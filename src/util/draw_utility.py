...
class Circle:
    def __init__(self, point: Point, radius: int = 8, color: ColorBGR = (0,0,255), thickness = -1) -> None:
    ...
    def draw_circle(self, opencv_frame: npt.NDArray):
        cv2.circle(opencv_frame, self.center_point.get_point_tuple(), self.radius, self.color, self.thickness, lineType=cv2.LINE_AA)
...
class DrawHandler:
    ...
    def is_resizeing(self, click: Point) -> bool:
        dist_start = np.linalg.norm(self.start_point.get_point_np() - click.get_point_np())
        dist_end = np.linalg.norm(self.end_point.get_point_np() - click.get_point_np())
