...
class ConnectedIndicator:
    def __init__(self, parent: tk.Misc, text: str = "Default") -> None:
        ...
        self.light = self.canvas.create_oval(2, 2, 18, 18, fill=self._colors[self._state])

        self.status_label = tk.Label(parent, text=self._text, font=("Arial", 12, "bold"))
    ...
    def update(self):
        color = self._colors.get(self._state, self._colors[ConnectionState.DISCONNECTED])
